...
- **Sync** (planned `crates/sync`): reconciles local journal with Jira/GitHub Issues; conflict policy (latest-write with surfaced diffs); offline queue and retry. CLI has `frodo sync` stub for now. Jira scope will be limited to project Issues with mapping to local tasks.
